      ...
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          ...
          try {
            ...
            text = (action === 'add' ? (chat.sWelcome || tradutor.texto1 || conn.welcome || 'Welcome, @user!').replace('@subject', await m?.conn?.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*CHECK DESCRIPTION*') :
...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await mconn.conn.reply(nk.from, `Hello *@${nk.from.split('@')[0]}*, las ${nk.isVideo ? 'calls' : 'calls'} are not allowed, you will be blocked.\n-\nIf you accidentally called contact my creator to unblock you!`, false, {mentions: [nk.from]});
