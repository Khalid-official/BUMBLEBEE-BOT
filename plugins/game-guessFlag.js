...
let handler = async (m, { conn, command, usedPrefix }) => {
    ...
    if (id in conn.tebakbendera) {
        conn.reply(m.chat, 'https://github.com/Khalid-official *[❗𝐈𝐍𝐅𝐎❗] There are still unanswered questions in this chat*', conn.tebakbendera[id][0])
        throw false
    }
...
*TYPE SURRENDER* for *surrender*
    ...
    conn.tebakbendera[id] = [
        ...
        json, poin,
        setTimeout(() => {
            if (conn.tebakbendera[id]) conn.reply(m.chat, `https://github.com/Khalid-official *[❗𝐈𝐍𝐅𝐎❗] Time's up!\nThe answer is* *${json.name}*`, conn.tebakbendera[id][0])
