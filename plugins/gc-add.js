...
const handler = async (m, {conn, text, participants, args}) => {
  if (!global.db.data.settings[conn.user.jid].restrict) throw ' *[❗] THE OWNER HAS RESTRICTED (ENABLE RESTRICT/DISABLE RESTRICT) THE USE OF THIS COMMAND*';
  ...
  try {
    ...
    for (const user of participant.filter((item) => item.attrs.error == 403)) {
      ...
      const teks = ` *[❗] IT WAS NOT POSSIBLE TO ADD @${jid.split('@')[0]}, THIS MAY HAPPEN BECAUSE THE NUMBER IS INCORRECT, THE PERSON HAS RECENTLY LEFT THE GROUP OR THE PERSON HAS CONFIGURED THEIR GROUP PRIV?CY, THE INVITATION TO THE GROUP WAS SENT TO THE USER IN THEIR PROVIDED*`;
    ...
    throw '*[❗] IT IS NOT POSSIBLE TO ADD THE NUMBER THAT YOU ENTERED, THIS MAY OCCUR BECAUSE THE NUMBER IS INCORRECT, THE PERSON HAS RECENTLY LEFT THE GROUP OR THE PERSON HAS CONFIGURED THEIR GROUP PRIV?CY, WE ADVISE YOU SEND THE INVITATION MANUALLY!!*';
  }
};
